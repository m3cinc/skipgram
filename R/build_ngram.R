...
#' @param filters is an optional character vector dictionary containing tokens to exclude, omit if no filter is supplied 
...
build_ngram <- compiler::cmpfun (function (x, gramname, gramdir, elimdir , maxn = 1L, sflag = FALSE, cflag = FALSE, filters = NULL) {
        ...
        for (n in 1L:maxn) {
                ...
                x %>% skipgram::skip_ngram( n, skip, cflag, filters) %>% unlist %>% table %>% sort(decreasing=TRUE) -> gram 
                filename <- paste0( gramdir,"/",gramname,"Gram", n, skip, ".RDS") ; saveRDS ( gram, file = filename)
                gram %>% extract(gram < 2L) %>% names -> discard ; filename <- paste0( elimdir, "/", gramname, "Discard", n, skip, ".RDS") ; saveRDS( discard, file = filename) ; rm( discard )
                gram %>% extract(gram > 1L) -> Vocabulary 
